...
let people = got.houses.reduce((acc, curr) => {
    return acc = acc.concat(curr.people);
}, [])


function getPeople(people) {
    container.innerHTML = people.map(el => {
        return `
            <div style='background-color: white; padding: 25px; text-align: center;'>
                <div style='display: flex; align-items: center;'>
                   <img style='border-radius: 50%;' src='${el.image}'/>
                   <p style='margin-left: 10px;'>${el.name}</p>
                </div>
                <p>${el.description}</p>
                <a style='display: block; text-align: center; background-color: rgb(231 237 253); text-decoration: none; padding: 5px 0; border-radius: 3px;' target='_blank' href=${el.wikiLink}>Learn More!</a>
             </div>
          `
    }).join('')
